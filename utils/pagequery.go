 ...
 */
func GetPagesInfo(totalItem int,currentpage int, pagesize int) (int) {
	if currentpage <= 1 {
		currentpage = 1
	}
	if pagesize == 0 {
...
func GetPagerLinks(totalItem int,po *PageOptions,context iris.Context) (int, html.HTML) {
	...
	return totalpages, html.HTML(str)
}

/**
 * 处理url,目的是保存参数
 */
func DealUri(po *PageOptions, ctx iris.Context) {
...
func fun4(po *PageOptions, totalpages int) string {
	...
	for i := totalpages - po.LinkItemCount; i <= totalpages; i++ {
		if po.Currentpage != i {
			rs += "<a href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(i) + "'>" + strconv.Itoa(i) + "</a>"
		} else {
			rs += "<span class=\"current\">" + strconv.Itoa(i) + "</span>"
		}
	}
...
func fun3(po *PageOptions, totalpages int) string {
	...
	rs += "<a href=''>...</a>"
	rs += "<a href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(totalpages) + "'>" + strconv.Itoa(totalpages) + "</a>"
	rs += getFooter(po, totalpages)
	return rs
...
func fun2(po *PageOptions, totalpages int) string {
	...
	for i := 1; i <= po.LinkItemCount+1; i++ {
		if i == po.LinkItemCount {
			rs += "<a href=\"" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(i) + "\">...</a>"
...
func getHeader(po *PageOptions, totalpages int) string {
	...
	if po.EnableFirstLastLink { //当首页,尾页都设定的时候,就显示
		...
		rs += "<a " + judgeDisable(po, totalpages, 0) + " href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(1) + "'>" + po.FirstPageText + "</a>"
	}
	if po.EnablePreNexLink { // disabled=\"disabled\"
		...
		rs += "<a " + judgeDisable(po, totalpages, 0) + " href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(a) + "'>" + po.PrePageText + "</a>"
	}
	return rs
}
...
func getFooter(po *PageOptions, totalpages int) string {
	...
	if po.EnablePreNexLink {
		...
		rs += "<a " + judgeDisable(po, totalpages, 1) + "  href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(a) + "'>" + po.NextPageText + "</a>"
	}
	if po.EnableFirstLastLink { //当首页,尾页都设定的时候,就显示
		rs += "<a " + judgeDisable(po, totalpages, 1) + " href='" + po.Href + "&" + po.ParamName + "=" + strconv.Itoa(totalpages) + "'>" + po.LastPageText + "</a>"
	}
	rs += "</div>"
	return rs
}
